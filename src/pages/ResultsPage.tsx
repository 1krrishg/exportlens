...
export default function ResultsPage() {
  ...
  // Prefer user's original search query as display title over USITC jargon description
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <main className="container mx-auto px-5 sm:px-6 py-10 max-w-3xl space-y-5">
          <div>
            ...
            <h1 className="text-2xl font-semibold text-foreground capitalize">{displayProductName}</h1>
          ...
          <div className="rounded-xl border-2 border-destructive/40 bg-destructive-soft p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-semibold text-destructive text-lg">
                  {result.sanctions_level === "prohibited" ? "Trade Prohibited" : "Trade Restricted"} — {result.sanctioned_party}
                </div>
                <div className="text-xs text-muted-foreground mt-0.5">Standard tariff analysis does not apply</div>
              </div>
            </div>
            <p className="text-sm text-foreground leading-relaxed">{result.sanctions_note}</p>
            <div className="rounded-lg border border-destructive/20 bg-background/50 p-3">
              ...
              <p className="text-xs text-foreground font-mono">{result.sanctions_authority}</p>
            </div>
            <div className="rounded-lg border border-warning/30 bg-warning-soft p-3">
              <p className="text-sm font-medium text-warning">{result.recommendation}</p>
            </div>
          </div>
  ...
  // Rate history is USITC US import data — only meaningful when destination IS the US
  const hasHistory = result.rate_history?.length > 1 && result.destination_country === "United States";
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="container mx-auto px-5 sm:px-6 py-10 max-w-3xl space-y-5">
        ...
        <div>
          ...
          <h1 className="text-xl sm:text-2xl md:text-3xl font-semibold text-foreground leading-snug capitalize">
            {displayProductName.length > 60 ? displayProductName.substring(0, 60) + "…" : displayProductName}
          </h1>
        ...
        <div className="rounded-xl border border-border bg-card divide-y divide-border overflow-hidden">
          ...
          {[
            ...
            { label: `${result.destination_country} MFN rate (all origins)`, value: result.mfn_rate !== null && result.mfn_rate !== undefined ? `${result.mfn_rate}%` : "N/A", note: result.mfn_rate !== null && result.mfn_rate !== undefined ? "WTO applied rate — what this country charges every trading partner today" : "WTO has no published rate for this corridor — effective rate uses available duty data only" },
            result.bound_rate !== null && result.bound_rate !== undefined
              ? { label: `${result.destination_country} avg WTO bound rate (all products)`, value: `${result.bound_rate}%`, note: result.escalation_headroom !== null && result.escalation_headroom !== undefined ? `Country-average legal ceiling under WTO commitments — on average ${result.destination_country} has ~${result.escalation_headroom}% headroom to raise tariffs without renegotiation. Product-specific bindings vary.` : "Country-average legal ceiling under WTO commitments. Product-specific bindings vary." }
              : null,
        ...
        {/* Trade intelligence — bilateral trade volume */}
        {result.bilateral_trade_value_m !== null && result.bilateral_trade_value_m !== undefined && (
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="px-4 py-3 border-b border-border bg-muted/30 flex items-center gap-2">
              <TrendingUp className="h-3.5 w-3.5 text-muted-foreground" />
              <div className="text-xs uppercase tracking-wider text-muted-foreground">Corridor trade activity</div>
            </div>
            <div className="p-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-2xl font-bold font-mono text-foreground">
                    ${result.bilateral_trade_value_m >= 1000
                      ? `${(result.bilateral_trade_value_m / 1000).toFixed(1)}B`
                      : `${result.bilateral_trade_value_m.toFixed(0)}M`}
                  </div>
                  <div className="text-xs text-muted-foreground mt-0.5">
                    {result.origin_country} → {result.destination_country} · HS {result.hs_code.substring(0, 4)} · UN Comtrade 2023
                  </div>
                </div>
                <div className={`text-xs px-2 py-1 rounded-full font-medium ${
                  result.bilateral_trade_value_m >= 1000 ? "bg-success-soft text-success" :
                  result.bilateral_trade_value_m >= 100 ? "bg-primary/10 text-primary" :
                  result.bilateral_trade_value_m >= 10 ? "bg-warning-soft text-warning" :
                  "bg-muted text-muted-foreground"
                }`}>
                  {result.bilateral_trade_value_m >= 1000 ? "High volume corridor" :
                   result.bilateral_trade_value_m >= 100 ? "Active corridor" :
                   result.bilateral_trade_value_m >= 10 ? "Moderate volume" :
                   "Low volume corridor"}
                </div>
              </div>
              <p className="text-xs text-muted-foreground mt-3 leading-relaxed">
                {result.bilateral_trade_value_m >= 1000
                  ? `This is a high-volume corridor — $${(result.bilateral_trade_value_m / 1000).toFixed(1)}B of this product moved on this route in 2023. Customs procedures are well-established and brokers are familiar with this trade lane.`
                  : result.bilateral_trade_value_m >= 100
                  ? `Active trade corridor with $${result.bilateral_trade_value_m.toFixed(0)}M in annual goods volume. Established customs procedures and freight infrastructure exist for this route.`
                  : result.bilateral_trade_value_m >= 10
                  ? `Moderate trade volume of $${result.bilateral_trade_value_m.toFixed(0)}M annually. This route is viable but may have fewer direct freight options than high-volume corridors.`
                  : `Low trade volume of $${result.bilateral_trade_value_m.toFixed(1)}M annually on this specific HS category. Verify freight availability — fewer carriers serve thin corridors.`
                }
              </p>
            </div>
          </div>
            ...
            <div className="mt-3 p-3 rounded-lg bg-muted/30 text-xs text-muted-foreground leading-relaxed">
              {result.volatility_stats?.max_jump_year
                ? <>Biggest single-year jump: <span className="text-destructive font-medium">+{result.volatility_stats.max_year_jump.toFixed(1)} percentage points</span> in {result.volatility_stats.max_jump_year}.
                  {(result.mfn_rate ?? 0) > 0 && result.volatility_stats.max_rate && (result.mfn_rate ?? 0) >= result.volatility_stats.max_rate - 1
                    ? " The MFN rate is currently at its historical peak."
                    : " The current MFN rate is below the historical peak."}</>
